...
namespace ViecLamViecNam.DAO
{
    public class Khachhang_trinhdo_nghiepvu_DAO
    {
        ...
        public static List<KhachHang_TrinhDo> Getlist_khachhangtrinhdo_Single(ModelCty db, int KH_id)
        {
        ...
        {
            var model = dbc.KhachHang_TimViec_TinHoc.Where(kh => kh.KH_ID == KH_id && 
        ...
        {
            var model = dbc.KhachHang_TimViec_NgoaiNgu_2022.Where(kh => kh.KH_ID == KH_id &&
                dbc.DM_NghiepVu.FirstOrDefault(a => a.NghiepVu_ID == kh.NghiepVu_NgoaiNgu_ID) != null);
            return model.ToList();
            //if (model != null) { return model.ToList(); } else { return null; }
        }
        public static string GetName_TV(ModelCty dbc, string ten, int KHID)
        {
            string name = "";
            if (ten == "HoTen")
            {
        ...
        private bool InsertTinhoc(int khachhangid, int tinhoc, int UserID)
        {
            try
            {
                ...
                kh.NguoiTao = UserID;
                dbc.KhachHang_TimViec_TinHoc.Add(kh);
                dbc.SaveChanges();
                return true;
            }
            catch (Exception ex)
        ...
        private bool InsertNgoaiNgu(int khachhangid, int ngoaingu, int UserID)
        {
            try
            {
                ...
                nn.NguoiTao = UserID;
                dbc.KhachHang_TimViec_NgoaiNgu_2022.Add(nn);
                dbc.SaveChanges();
                return true;
            }
            catch (Exception ex)
        ...
        {
            var XoaNLD = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[KhachHang_NghiepVu] where KH_ID=" + khachhangid);
            var XoaNLD2 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[KhachHang_TimViec_NgoaiNgu_2022] where KH_ID=" + khachhangid);
            var XoaNLD3 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[KhachHang_TimViec_TinHoc] where KH_ID=" + khachhangid);
            if (NghiepVu1 != 0)
            {
        ...
        public bool Xoa_InsertKhachHang_trinhdo(int TrinhDo_ID, int Nganh_ID,int Nghe_ID ,int khachhangid, int UserID)
        {
            ...
            {
                var XoaNLD_TrinhDo = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[KhachHang_TrinhDo] where KH_ID=" + khachhangid);
