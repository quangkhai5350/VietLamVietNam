...
namespace ViecLamViecNam.DAO
{
    public class TD_NghiepVu_TinHoc_NgoaiNgu_Dao
    {
        ...
        {
            var XoaNLD = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[DoanhNghiep_TuyenDung_NghiepVu] where TuyenDung_ID=" + TuyenDung_ID);
            var XoaNLD2 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[DoanhNghiep_TuyenDung_NgoaiNgu] where TuyenDung_ID=" + TuyenDung_ID);
            var XoaNLD3 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[DoanhNghiep_TuyenDung_TinHoc] where TuyenDung_ID=" + TuyenDung_ID);
            if (NghiepVu1 != 0)
            {
