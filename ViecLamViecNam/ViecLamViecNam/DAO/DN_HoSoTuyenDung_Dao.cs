...
using ViecLamViecNam.Models.table_mirro;
...
namespace ViecLamViecNam.DAO
{
    public class DN_HoSoTuyenDung_Dao
    {
        ...
        //private static List<DoanhNghiep_TuyenDung> LinQ_DN_TD_StrAPI(ModelCty db, string str)
        //{
        ...
        //}
        public static List<DoanhNghiep_TuyenDung> LinQ_DN_TD(ModelCty db)
        {
            var model = db.Database.SqlQuery<DoanhNghiep_TuyenDung>("exec GetBase_DN_TD_khai").ToList();
            return model;
        }
        public static List<DoanhNghiep_TuyenDung> LinQ_DN_TD_Pay(ModelCty db)
        {
            var model = db.Database.SqlQuery<DoanhNghiep_TuyenDung>("exec GetBase_DN_TD_khai_Pay").ToList();
            return model;
        }
        public static List<DoanhNghiep_TuyenDung> LinQ_Job_left(ModelCty db)
        {
        ...
        public static List<DoanhNghiep_TuyenDung> LinQ_Job_left_Pay(ModelCty db)
        {
...
//Lấy tất cả cho MainJob
        public List<DoanhNghiep_TuyenDung> GetListTDbyNghanhNghe(int sec, bool key, int pageSize, string searchId)
        {
            ...
            else
            {
                ...
                var modelmapSub = db.aspnet_mapSub.Where(kh => kh.mapautoId == nn)
                        .Select(kh => kh.Substr)
                        .ToList();
                if (modelmapSub != null)
                {
        ...
        {
            var model = db.aspnet_mapauto.ToList();
            return model;
        }
        ...
        public List<DoanhNghiep_TuyenDung> GetmapTDbyTieudeTVL(int KH_ID, bool key)
        {
            ...
            for (int i = 0; i < modelallmap.Count(); i++)
            {
                ...
                var modelmapSub = db.aspnet_mapSub.Where(kh => kh.mapautoId == id)
                    .Select(kh => kh.Substr)
                    .ToList();
                if (modelmapSub != null)
                {
        ...
        private static List<DoanhNghiep_TuyenDung> LinQ_TDbyDN(ModelCty db, int DNID)
        {
        ...
        {
            var model = db.SMS_admintoDN.Where(kh => kh.DNID == DNID && kh.DaXem ==false).Count();
            return model;
        }
