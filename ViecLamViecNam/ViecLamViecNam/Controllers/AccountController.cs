    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult _JobCategory()
        {
            if (Session["UsrID"] != null)
            {
                ...
                ViewBag.TotalDaLuu = dbc.Database.SqlQuery<PHU_HSTV_Luu>("select t1.Id,t1.UserId,t1.NTV_Id,t1.HSTV_Id,t1.TenHoSo,t1.NgayTao " +
                    "from [VLCty].[dbo].[PHU_HSTV_Luu] t1 " +
                    "inner join " +
                    "[VLCty].[dbo].[KhachHang_TimViecLam] t2 on t1.HSTV_Id = t2.TimViec_ID" +
        ...
        public ActionResult _JobInfo()
        {
            if (Session["UsrID"] != null)
            {
                ...
                if (DN_ID != null)
                {
                    ViewBag.DoanhNghiep = dbc.Database.SqlQuery<DoanhNghiep>("select * from [VLCty].[dbo].[DoanhNghiep] where DN_ID=" + DN_ID.UserChild_id);
                    return PartialView();
                }
                else
                {
        ...
        public ActionResult _JobDoc()
        {
            if (Session["UsrID"] != null)
            {
                ...
                if (DN_ID != null)
                {
                    ...
                    var DNId = dbc.Database.SqlQuery<int>("select DN_ID from [VLCty].[dbo].[DoanhNghiep] where DN_ID=" + DN_ID.UserChild_id);
                    ...
                    {
                        var dsTongdiem = dbc.Pay_TonCuoi.FirstOrDefault(kh => kh.DNID == DNId.FirstOrDefault());
                        ViewBag.Tongdiem = (dsTongdiem != null) ? dsTongdiem.SoDiemTon : 0;
                        var dsgiasp = dbc.Pay_GiaSP.Find(1);
        ...
        public ActionResult _JobManagement()
        {
            if (Session["UsrID"] != null)
            {
                ...
                for(int i= 0;i< listTDbyDN.Count; i++)
                {
                    ...
                    var item = dbc.DoanhNghiep_UngTuyen.FirstOrDefault(kh => kh.TuyenDung_ID == num);
                    if (item != null)
                    {
        ...
        public ActionResult DeleteJobDoc(int Id)
        {
            ...
            if (model != null)
            {
                try
                {
                    var XoaNLD = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[DoanhNghiep_TuyenDung_NghiepVu] where TuyenDung_ID=" + model.TuyenDung_ID);
                    var XoaNLD2 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[DoanhNghiep_TuyenDung_NgoaiNgu] where TuyenDung_ID=" + model.TuyenDung_ID);
                    var XoaNLD3 = dbc.Database.ExecuteSqlCommand("DELETE  FROM [VLCty].[dbo].[DoanhNghiep_TuyenDung_TinHoc] where TuyenDung_ID=" + model.TuyenDung_ID);
        ...
        public ActionResult UpdateDocFile(DoanhNghiep_TuyenDung model, DateTime? NgayNhanHoSo1, DateTime NgayHetHan1, int NghiepVu1, int NghiepVu11, int NghiepVu2, int NghiepVu22, int NghiepVu3, string Tieude_kh)
        {
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var update = dbc.Database.ExecuteSqlCommand("update VLCty.dbo.[DoanhNghiep_TuyenDung] set [TieuDeTuyenDung]=@TenHoSo," +
        ...
        public ActionResult thaydoitrangthai(int tuyendungid)
        {
            ...
            var update = dbc.Database.ExecuteSqlCommand("update VLCty.dbo.[DoanhNghiep_TuyenDung] set HienThiWeb=@HienThiWeb where TuyenDung_ID=@Id",
        ...
        //                    var update = dbc.Database.ExecuteSqlCommand("update VLCty.dbo.[DoanhNghiep_TuyenDung] set KichHoat=0 where TuyenDung_ID=@Id",
        //                new SqlParameter("@Id", tuyendungid));
        ...
        public ActionResult Guipheduyet(int tuyendungid)
        {
            if (Session["UsrID"] != null)
            {
                if ((bool)Session["Pay"] == true)
                {
                    ...
                    var UserWeb = dbc.UserWebs.FirstOrDefault(kh => kh.UserID == UsId && kh.UserRoles_NVLoaitaikhoan == 4);
                    //thêm mới CTXuatDiem
                    var CTXuat = dbc.Pay_CTXuatDiem.Where(kh => kh.Idsp == tuyendungid.ToString() && kh.DNID == UserWeb.UserChild_id);
                    if (CTXuat.Count() == 0)
                    {
                        var dsgiasp = dbc.Pay_GiaSP.Find(1);
                        Pay_CTXuatDiem model = new Pay_CTXuatDiem();
                        ...
                        model.ID_giaSP = 1;
                        model.Ngay = DateTime.Now;
                        dbc.Pay_CTXuatDiem.Add(model);
                        var kq = dbc.SaveChanges();
                        if (kq > 0)
                        {
                            //update tồn cuối
                            var xuatdiem = dbc.Pay_CTXuatDiem.Where(kh => kh.DNID == UserWeb.UserChild_id).ToList();
                            double tongxd = 0;
                            for (int i = 0; i < xuatdiem.Count; i++)
                            {//cộng tất cả điểm xuất
                                var giasp = dbc.Pay_GiaSP.Find(xuatdiem[i].ID_giaSP);
                                tongxd = tongxd + giasp.Gia;
                            }
                            var diemton = dbc.Pay_TonCuoi.Find(UserWeb.UserChild_id);
                            if (diemton != null)
                            {//update Pay_toncuoi
                                diemton.TongXuat = tongxd;
                                diemton.SoDiemTon = diemton.TongNhap - tongxd;
                                ...
                                if (kup > 0)
                                {
                                    var dsTongdiem = dbc.Pay_TonCuoi.FirstOrDefault(kh => kh.DNID == UserWeb.UserChild_id);
                                    ViewBag.Tongdiem = (dsTongdiem != null) ? dsTongdiem.SoDiemTon : 0;
                                    //update kickhoat==False//13/06 khong can
                                    var tuyendung = dbc.DoanhNghiep_TuyenDung.FirstOrDefault(kh => kh.TuyenDung_ID == tuyendungid);
                                    if (tuyendung != null)
                                    {
                                        var update = dbc.Database.ExecuteSqlCommand("update VLCty.dbo.[DoanhNghiep_TuyenDung] set TinhTrangHoSo=2 where TuyenDung_ID=@Id",
                                            new SqlParameter("@Id", tuyendungid));
                                        if (update > 0)
                                        {
        ...
        public ActionResult _CandidateCategory()
        {
            if (Session["UsrID"] != null)
            {
                ...
                ViewBag.TotalHSDaLuu = dbc.Database.SqlQuery<PHU_HSTD_Luu>("select * from [VLCty].[dbo].[PHU_HSTD_Luu] where UserId =" + uID
        ...
        public ActionResult _CandidateDoc()
        {
            if (Session["UsrID"] != null)
            {
                ...
                if (KH_ID != null)
                {
                    var NTVId = dbc.Database.SqlQuery<int>("select KH_ID from [VLCty].[dbo].[KhachHang] where KH_ID=" + KH_ID.UserChild_id);
        ...
        public ActionResult _CandidateInfo()
        {
            if (Session["UsrID"] != null)
            {
                ...
                ViewBag.NTV = dbc.Database.SqlQuery<KhachHang>("select * from [VLCty].[dbo].[KhachHang] where KH_ID=" + user.UserChild_id);
                return PartialView();
            }
            return Json("No", JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var model = dbc.KhachHang_KinhNghiem_LamViec_2022.Find(Id);
            if (model != null)
            {
                try
                {
                    dbc.KhachHang_KinhNghiem_LamViec_2022.Remove(model);
                    dbc.SaveChanges();
        ...
        public ActionResult thaydoitrangthai_kh(int tuyendungid)
        {
            ...
            var update = dbc.Database.ExecuteSqlCommand("update VLCty.dbo.[KhachHang_TimViecLam] set HienThiTrenWeb=@HienThiTrenWeb where TimViec_ID=@Id",
        ...
        public ActionResult _CandidateManagement()
        {
            if (Session["UsrID"] != null)
            {
                ...
                ViewBag.HSDaLuu = dbc.Database.SqlQuery<PHU_HSTD_Luu>("select * from [VLCty].[dbo].[PHU_HSTD_Luu] where UserId =" + uID
        ...
        public ActionResult DN_tuchoiungvien(int KH_ID = 0, int tuyendung_ID = 0, int tuchoi = 2)
        {
            ...
            var item = dbc.DoanhNghiep_UngTuyen.FirstOrDefault(kh => kh.TuyenDung_ID == tuyendung_ID && kh.KH_ID == KH_ID);
            if (item != null)
            {
                item.NgayUpdate = DateTime.Now;
                item.NTV_LyDoHuy = "NO";
        ...
        public ActionResult ThemKinhNghiem(String TenCongTy, String ChucVu, DateTime TuThangCV, DateTime DenThangCV, String CongViecChinh, String DaLamViecNuocNgoai)
        {
            if (Session["UsrID"] != null)
            {
                ...
                model.NgayTao = DateTime.Now;
                model.NguoiTao = uID;
                dbc.KhachHang_KinhNghiem_LamViec_2022.Add(model);
                int kt = dbc.SaveChanges();
                if (kt != 0)
                {
