    ...
    public class JobController : Controller
    {
        ...
        public ActionResult MainJob(int key, string searchNN = "")
        {
            ...
            if(searchNN != "")
            {
                ...
                var mapaotu = dbc.aspnet_mapauto.Find(ii);
        ...
        public ActionResult SMStoDN()
        {
            if (Session["UsrID"] != null)
            {
                ...
                if (DN_ID != null)
                {
                    ViewBag.SMS_toDN = dbc.SMS_admintoDN.Where(kh => kh.DNID == DN_ID.UserChild_id)
        ...
        {
            var timviec = dbc.SMS_admintoDN.Find(new Guid(id));
            if (timviec.DaXem == false)
            {
                var update = dbc.Database.ExecuteSqlCommand("update VLCty.dbo.[SMS_admintoDN] set DaXem=@daxem where Id=@Id",
